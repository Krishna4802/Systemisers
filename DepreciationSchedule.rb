module ProjectReport
    class DepreciationSchedule
      ...
      def add_asset(name, depreciation_percent:, opening_balance: 0, addition: 0, is_less_than_6_months: false)
        @assets << {
          name: name,
          depreciation_percent: depreciation_percent, # Store the depreciation percent for each asset
      ...
      def display_schedule
        ...
        @assets.each do |asset|
          asset_details << {
            name: asset[:name],
            depreciation_percent: asset[:depreciation_percent], # Include depreciation percent in output
