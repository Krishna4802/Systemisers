  ...
  class GenerateDepreciationSchedulePdf
    ...
    def generate_pdf(output_path = "Depreciation_Schedule_Report.pdf")
      Prawn::Document.generate(output_path) do |pdf|
        ...
        pdf.text "Diddi Computer Trading", size: 16, style: :bold
        pdf.text "Hyderabad", size: 12
        pdf.move_down 20

        pdf.text "DEPRECIATION SCHEDULE", size: 16, style: :bold
        pdf.move_down 10
        pdf.text "Rs. in Lakhs", size: 12
        pdf.move_down 10
        ...
        table_data = []
        headers = ["PARTICULARS", "Projected Period"] + years.map { |year| "#{year}-#{year + 1}" }
        table_data << headers
        ...
        @schedule.assets.each do |asset|
          ...
          table_data << [asset[:name]] # Asset name
          ...
          table_data << [] # Add a blank row for spacing
        end
